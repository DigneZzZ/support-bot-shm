...
async def clear_fsm_keys(redis_url: str):
    """
    Очистка только ключей FSMContext в Redis.

    :param redis_url: str: URL подключения к Redis.
    """
    redis = await aioredis.from_url(redis_url)
    keys = await redis.keys("fsm:*")  # Получаем все ключи FSMContext
    if keys:
        await redis.delete(*keys)  # Удаляем их
    await redis.close()
...
async def on_startup(
    ...
    """
    # Start apscheduler
    print("⚡️ Бот запускается...")  # Проверка
    apscheduler.start()
    # Setup commands when starting up
    await commands.setup(bot, config)
    print("✅ Команды загружены!")  # Проверка
...
async def main() -> None:
    ...
    # Очистка только FSM ключей перед запуском бота
